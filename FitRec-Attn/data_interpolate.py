...
class dataInterpreter(object):
    ...
    def preprocess_data(self):
        ...
        self.processed_path = self.data_path + \
        ...
        if os.path.exists(self.processed_path):
            ...
            self.original_data = np.load(
                self.processed_path, allow_pickle=True)[0]
            self.map_workout_id()
        else:
    ...
    def smooth(self, x, ws):  # ws: window size
        return np.mean(x.reshape(-1, ws), axis=1)
    ...
    def dataIteratorSupervised(self, trainValidTest):
        ...
        for idx in indices:
            ...
            if self.includeUser:
                user_inputs = np.ones(
                    [num_steps]) * self.oneHotMap['userId'][current_input['userId']]
            if self.includeSport:
                sport_inputs = np.ones(
                    [num_steps]) * self.oneHotMap['sport'][current_input['sport']]
            ...
            if self.includeTemporal:
                ...
                context_since_last = np.ones(
                    [1, num_steps]) * self.contextMap[idx][0]
                ...
                for att_idx, att in enumerate(inputAtts):
                    if att == 'time_elapsed':
                        ...
                        context_inputs[att_idx,
                                       :] = context_input[att][:num_steps]
                    else:
                        context_inputs[att_idx,
                                       :] = context_input[att][:num_steps]
                for att in targetAtts:
                    context_outputs[0, :] = context_input[att][:num_steps]
            ...
            # total time - T segments with window size T
            for t in range(num_steps - self.T):
                inputs_dict_t = {}
    ...
    def generator_for_autotrain(self, batch_size, num_steps, trainValidTest):
        ...
        if trainValidTest == "train":
            data_len = sum([(len(self.original_data[idx]['distance']) - self.T)
            ...
            data_len = sum(
                [(len(self.original_data[idx]['distance']) - self.T) for idx in self.testSet])
        else:
        ...
        for i in range(epoch_size):
            ...
            if self.includeTemporal:
                context_input_1 = np.zeros(
                    [batch_size, self.T, inputDataDim + 1])
...
if __name__ == "__main__":
    ...
    inputAtts = ["derived_speed", 'altitude']
    ...
    trainValidTestFN = data_path.split(".")[0] + "_temporal_dataset.pkl"
    zMultiple = 5
    includeUser = True
