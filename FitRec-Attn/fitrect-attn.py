...
class contextEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.context_layer_2 = nn.LSTM(
            input_size=1, hidden_size=self.hidden_dim, batch_first=True)
    ...
    def forward(self, context_input_1, context_input_2):
        ...
        hidden_1 = self.init_hidden(context_input_1)
        cell_1 = self.init_hidden(context_input_1)
        # 1 * batch_size * hidden_size
        hidden_2 = self.init_hidden(context_input_2)
        ...
        outputs_1, lstm_states_1 = self.context_layer_1(
        ...
        outputs_2, lstm_states_2 = self.context_layer_2(
        ...
        context_embedding = self.project(
            torch.cat([context_embedding_1, context_embedding_2], dim=-1))
...
class encoder(nn.Module):
    def __init__(self, input_size, hidden_size, T, attr_embeddings, logger, dropout=0.1):
        ...
        self.attn_linear = nn.Linear(
            in_features=2 * hidden_size + T, out_features=1)
    ...
    def forward(self, attr_inputs, context_embedding, input_variable):
        ...
        input_weighted = Variable(input_data.data.new(
            input_data.size(0), self.T, self.input_size).zero_())
        input_encoded = Variable(input_data.data.new(
            input_data.size(0), self.T, self.hidden_size).zero_())
        ...
        for t in range(self.T):
            ...
            x: torch.Tensor = self.attn_linear(
                x.view(-1, self.hidden_size * 2 + self.T))  # (batch_size * input_size) * 1
...
class decoder(nn.Module):
    def __init__(self, encoder_hidden_size, decoder_hidden_size, T, logger):
        ...
        self.lstm_layer = nn.LSTM(
            input_size=1, hidden_size=decoder_hidden_size)
...
class da_rnn:
    def __init__(self, logger, encoder_hidden_size=64, decoder_hidden_size=64, T=10,
        ...
        print("include pretrain/user/sport/temporal = {}/{}/{}/{}".format(self.pretrain,
              self.includeUser, self.includeSport, self.includeTemporal))
        ...
        if self.includeTemporal:
            ...
            self.context_encoder = contextEncoder(
                input_size=self.input_dim + 1, hidden_size=encoder_hidden_size, output_size=self.context_dim).cuda()
        ...
        self.encoder_optimizer = optim.Adam([
            ...
            ) if 'user_embedding' in name], 'weight_decay':wd1},
            {'params': [param for name, param in self.encoder.named_parameters(
            ) if 'sport_embedding' in name], 'weight_decay':wd2},
            {'params': [param for name, param in self.encoder.named_parameters(
    ...
    def train(self, n_epochs=30, print_every=400):
        ...
        for iteration in range(n_epochs):
            ...
            trainDataGen = self.endo_reader.generator_for_autotrain(
                self.batch_size, self.num_steps, "train")
            print_loss = 0
            for batch, training_batch in enumerate(trainDataGen):
                ...
                loss = self.train_iteration(
                    attr_inputs, context_input_1, context_input_2, input_variable, y_history, y_target)
            ...
            validDataGen = self.endo_reader.generator_for_autotrain(
                self.batch_size, self.num_steps, "valid")
            val_loss = 0
            val_batch_num = 0
            ...
            if not best_val_loss or val_loss <= best_val_loss:
                ...
                best_epoch_path = self.model_save_location + \
        ...
        testDataGen = self.endo_reader.generator_for_autotrain(
            self.batch_size, self.num_steps, "test")
        test_loss = 0
        test_batch_num = 0
        ...
        for test_batch in testDataGen:
            ...
            attr_inputs, context_input_1, context_input_2, input_variable, y_history, y_target = self.get_batch(
                test_batch)
            loss = self.evaluate(attr_inputs, context_input_1,
                                 context_input_2, input_variable, y_history, y_target)
    ...
    def train_iteration(self, attr_inputs, context_input_1, context_input_2, input_variable, y_history, y_target):
        ...
        context_embedding = self.context_encoder(
            context_input_1, context_input_2)
        input_weighted, input_encoded = self.encoder(
            attr_inputs, context_embedding, input_variable)
        ...
        while i < len(y_pred):
            ...
            for j in range(len(batch_idx)):
                if on_train:
                    ...
                    X[j, :, :] = self.X[range(
                        batch_idx[j] + self.train_size - self.T, batch_idx[j] + self.train_size - 1), :]
                    y_history[j, :] = self.y[range(
                        batch_idx[j] + self.train_size - self.T,  batch_idx[j] + self.train_size - 1)]

            y_history = Variable(torch.from_numpy(
                y_history).type(torch.FloatTensor).cuda())
...
#T=20
T = 10
print("learning rate = {}, batch_size = {}, hidden_size = {}, T = {}".format(
    learning_rate, batch_size, hidden_size, T))
model = da_rnn(logger=logger, parallel=False, T=T, encoder_hidden_size=hidden_size,
               decoder_hidden_size=hidden_size, learning_rate=learning_rate, batch_size=batch_size)
